...
def tau_set(n: int):
    ...
    for k in k_list:
        ...
        tau.append({"gate": gate, "t_count": cost, "id": gate_to_id[f'Rz({k}*π/{denom})'], "magic_gate": True})
    return tau
...
def unitary_from_key(db, key, gate_matrix):
    ...
    # Collect gate ids from this node back to the root
    gate_ids = []
    while idx != -1:
    ...
    # Reverse so they go root -> ... -> target
    gate_ids.reverse()
...
def closest_neighbors(unitary, db, nn, epsilon, base_gate_set):
    r = np.sqrt(2) * epsilon
    idxs = nn.radius_neighbors(np.array(key_from_U(unitary), dtype=float).reshape(1, -1), radius=r, return_distance=False)
    ...
    for i in idxs[0]:
        temp_distance = distance(np.array(db.keys[i], dtype=float), np.array(key_from_U(unitary), dtype=float))
        
        if temp_distance <= epsilon:
            temp_magic = db.magic_count[i]
            if best_magic is None or temp_magic < best_magic or (temp_magic == best_magic and temp_distance < best_distance):
                best_magic = temp_magic
                best_distance = temp_distance
                best_unitary, best_seqs = unitary_from_key(db, db.keys[i], gate_matrix)
